...
def analyze_gamestate_for_heuristic_maneuver(game_state, ship_state):
    ...
    def find_largest_gap(asteroids, ship_position):
        ...
        angles = []
        initial_cover_count = 0  # Counter for asteroids covering angle 0
    ...
    if most_imminent_asteroid is None:
        most_imminent_asteroid_speed = 0
        imminent_asteroid_relative_heading_deg = 0
    else:
    ...
    if nearby_asteroid_count == 0:
        nearby_asteroid_average_speed = 0
    else:
...
class Simulation():
    ...
    def get_instantaneous_asteroid_collision(self, asteroids: Optional[list] = None, ship_position: Optional[tuple] = None):
        ...
        def get_mine_safety_fitness(next_extrapolated_mine_collision_times: list):
            ...
            if next_extrapolated_mine_collision_times:
                for mine_collision_time, mine_pos in next_extrapolated_mine_collision_times:
                    ...
                    mine_safe_time_score += 15 - 5*mine_collision_time/3 + max(0.0, mine_ground_zero_fudge)
    ...
    def bullet_sim(self, ship_state: Optional[dict] = None, fire_first_timestep: bool = False, fire_after_timesteps: int = 0, skip_half_of_first_cycle: bool = False, current_move_index: Optional[int] = None, whole_move_sequence: Optional[list] = None, timestep_limit: int = INT_INF):
    ...
    def apply_move_sequence(self, move_sequence: Optional[list] = None):
    ...
    def cruise(self, cruise_time: int, cruise_turn_rate=0):
        # Maintain current speed
...
class NeoController(KesslerController):
    ...
    def __init__(self, chromosome: Optional[list] = None):
        debug_print('INITIALIZING NEO')
        self.reset(chromosome)

    def reset(self, chromosome: Optional[list] = None):
    ...
    def enqueue_action(self, timestep: int, thrust: float=0.0, turn_rate: float=0.0, fire: bool=False, drop_mine: bool=False):
    ...
    def decide_next_action(self, game_state: dict, ship_state: dict):
    ...
    def plan_action(self, other_ships_exist: bool, base_state_is_exact: bool, iterations_boost: bool=False, plan_stationary: bool=False):
        ...
        if self.game_state_to_base_planning['respawning']:
            ...
            for _ in range(search_iterations):
                ...
                if not heuristic_maneuver:
                    maneuver_fitness -= 0 # TODO: REMOVE FUDGE TO ENABLE RANDOM SEARCH AGAIN
                else:
                    maneuver_fitness += 0
                self.sims_this_planning_period.append({
                    'sim': maneuver_sim,
